...
function criarGraficos() {
    ...
    chartSeveridade = new Chart(
        document.getElementById('chart-severidade').getContext('2d'),
        {
            type: 'bar',
            data: {
                labels: ['Info', 'Aviso', 'Crítico', 'Sensor'],
                datasets: [{
                    label: 'Qtd.',
                    data: [0, 0, 0, 0],
                    backgroundColor: ['#3498db', '#e67e22', '#dd0000', '#999999'],
                    borderColor: [COR_INFO, COR_AVISO, COR_PERIGO, COR_SENSOR],
                    borderWidth: 1.5,
                    borderRadius: 3
                }]
            },
...
async function fetchAlertasRecentes() {
    try {
        ...
        titulo.textContent = `Alertas recentes (${data.length})`;
